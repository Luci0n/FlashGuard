...
function Read-Candidate([object]$Spec, [string]$BatchDir) {
    ...
    $motion = Get-Content -Raw (Join-Path $dir 'motion-diagnostics.json') | ConvertFrom-Json
    ...
    $movingDiag = $motion.cases.moving_square
    [pscustomobject][ordered]@{
        name=[string]$Spec.name; architecture_mode=[int]$Spec.architecture_mode
        risk_neutral=$Spec.risk_neutral; risk_gain=$Spec.risk_gain; replay_status=[string]$replay.status
        ...
        moving_diag_current_surface_active=[double]$movingDiag.current_surface.active_fraction
        moving_diag_vacated_active=[double]$movingDiag.vacated_surface.active_fraction
        moving_diag_infill_active=[double]$movingDiag.disocclusion_infill.active_fraction
        moving_diag_hardware_active=[double]$movingDiag.hardware_combined.active_fraction
    }
}
...
$neutral12 = Merge-Tune $lowContrast @{risk_neutral=.12}

$screenSpecs = @(
    (New-Spec 'legacy_default' 0),
    (New-Spec 'risk_only_v1_default' 1),
    (New-Spec 'surface_risk_v2_neutral12' 3 $neutral12),
    (New-Spec 'seed_veto_default' 4),
    (New-Spec 'seed_veto_low_contrast' 4 $lowContrast),
    (New-Spec 'seed_veto_neutral12' 4 $neutral12),
    (New-Spec 'seed_veto_neutral12_gain100' 4 (Merge-Tune $neutral12 @{risk_gain=1.0})),
    (New-Spec 'seed_veto_neutral12_gain110' 4 (Merge-Tune $neutral12 @{risk_gain=1.1})),
    (New-Spec 'seed_veto_neutral12_gain120' 4 (Merge-Tune $neutral12 @{risk_gain=1.2}))
)

Write-Host "FlashBench disocclusion-seed-veto screen: $($screenSpecs.Count) configurations"
...
$report = [ordered]@{
    schema='FLASHGUARD_MATRIX/9'; purpose='disocclusion false-event seed veto experiment'
    architecture_modes=[ordered]@{
        '0'='legacy transported protected luminance'
        '1'='risk-only v1 with screen-space risk memory'
        '3'='surface-risk v2; disocclusion may seed a fresh intrinsic event'
        '4'='surface-risk v3; disocclusion cannot seed fresh surface risk'
    }
...
Write-Host "Selected seed-veto candidate: $($selected.name) (failed gates=$($selected.failed_architecture_gates))"
exit 0
