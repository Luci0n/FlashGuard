...
function Invoke-Batch([object[]]$Specs) {
    ...
    $sw.Stop()
    if ($p.ExitCode -ne 0) { throw "Corrected architecture matrix failed with exit code $($p.ExitCode)" }
...
$specs = @(
    ...
    (New-Spec 'surface_propagation_tau100_gain200' 9 (Merge-Tune $base $tune)),
    (New-Spec 'single_frame_phase_hold_tau100_gain200' 10 (Merge-Tune $base $tune)),
    (New-Spec 'textureless_flow_consensus_tau100_gain200' 11 (Merge-Tune $base $tune)),
    (New-Spec 'uniform_interior_intrinsic_tau100_gain200' 12 (Merge-Tune $base $tune))
)

Write-Host "FlashBench self-validating architecture matrix: $($specs.Count) configurations"
$screen = Invoke-Batch $specs
$candidates = @($specs | ForEach-Object { Read-Candidate $_ $screen.directory })
$selected = @($candidates | Where-Object { $_.architecture_mode -ge 9 -and $_.primary_pass } | Select-Object -First 1)
$report = [ordered]@{
    schema='FLASHGUARD_MATRIX/21'
    purpose='first self-validating comparison of requested architectures 8-12 after fixing the replay-batch architecture clamp'
    screening_resolution='320x180'
    screening_fps=30
    screen_elapsed_ms=$screen.elapsed_ms
    candidate_count=$candidates.Count
    invariant='production architecture 0 is unchanged; replay parser accepts requested modes through 12 and rejects out-of-range requests instead of silently clamping'
    prior_invalidated_matrices=@('FLASHGUARD_MATRIX/17','FLASHGUARD_MATRIX/18','FLASHGUARD_MATRIX/19','FLASHGUARD_MATRIX/20')
...
Write-Host "Self-validating architecture matrix complete in $($screen.elapsed_ms) ms"
exit 0
