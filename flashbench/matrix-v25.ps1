...
function Invoke-Batch([object[]]$Specs) {
    ...
    $args = @('--synthetic-replay-batch',('"'+$plan+'"'),'--synthetic-replay-batch-output',('"'+$dir+'"'),'--replay-width','320','--replay-height','180')
    $sw = [Diagnostics.Stopwatch]::StartNew()
    $p = Start-Process -FilePath $exe -ArgumentList $args -Wait -PassThru
    $sw.Stop()
    if ($p.ExitCode -ne 0) { throw "Full finalist matrix failed with exit code $($p.ExitCode)" }
...
function Read-Candidate([object]$Spec, [string]$BatchDir) {
    ...
    [pscustomobject][ordered]@{
        ...
        smooth_subpixel_scroll_mae=[double]$replay.smooth_subpixel_scroll_mae
        integer_snapped_scroll_mae=[double]$replay.integer_snapped_scroll_mae
        wcag_sc_2_3_1_pass=$wcag231
        wcag_sc_2_3_2_pass=$wcag232
        primary_pass=($behaviorPass -and $clear -le 70.0 -and $pan -lt 0.010 -and
            [double]$replay.fast_pan_mae -lt 0.020 -and
            [double]$replay.extreme_pan_mae -lt 0.030 -and
...
Write-Host "FlashBench full finalist matrix: $($specs.Count) configurations"
...
$report = [ordered]@{
    schema='FLASHGUARD_MATRIX/25'
    purpose='canonical full-replay validation of camera-aware event-only disocclusion correction after Matrix 24 finalists exposed fast-pan regressions hidden by screening selection'
    screening_resolution='320x180'
    screening_fps=30
    screen_elapsed_ms=$screen.elapsed_ms
    candidate_count=$candidates.Count
    invariant='production architecture 0 is unchanged; mode 16 inherits mode 14 raw history/residual rejection and corrected current-event disocclusion, but coherent whole-frame/camera motion blends the event interpretation back toward raw disocclusion before current intrinsic authority is admitted'
    primary_gate=[ordered]@{
        replay_status_required='SUCCESS'
        pan_mae_max_exclusive=0.010
        fast_pan_mae_max_exclusive=0.020
        extreme_pan_mae_max_exclusive=0.030
...
Write-Host "Full finalist matrix complete in $($screen.elapsed_ms) ms"
exit 0
