...
function Invoke-Batch([object[]]$Specs) {
    ...
    $sw.Stop()
    if ($p.ExitCode -ne 0) { throw "Bounded-risk matrix failed with exit code $($p.ExitCode)" }
...
function Read-Candidate([object]$Spec, [string]$BatchDir) {
    ...
    [pscustomobject][ordered]@{
        ...
        primary_gate_pass=([bool]$trail.moving_square_clear_to_0_05_observed -and $clear05 -le 70.0 -and $minPerceptual -ge 0.70 -and $movingReduction -ge 0.45 -and $sc231 -and $sc232)
    }
}
...
$specs = @(
    ...
    (New-Spec 'surface_risk_v2_default' 3 $lowContrast),
    (New-Spec 'event_only_gain120' 6 (Merge-Tune $lowContrast @{risk_gain=1.2})),
    (New-Spec 'event_only_gain160' 6 (Merge-Tune $lowContrast @{risk_gain=1.6})),
    (New-Spec 'event_only_gain200' 6 (Merge-Tune $lowContrast @{risk_gain=2.0})),
    (New-Spec 'bounded_tau010_gain160' 3 (Merge-Tune $lowContrast @{surface_risk_tau=.010; risk_gain=1.6})),
    (New-Spec 'bounded_tau020_gain160' 3 (Merge-Tune $lowContrast @{surface_risk_tau=.020; risk_gain=1.6})),
    (New-Spec 'bounded_tau030_gain160' 3 (Merge-Tune $lowContrast @{surface_risk_tau=.030; risk_gain=1.6})),
    (New-Spec 'bounded_tau040_gain160' 3 (Merge-Tune $lowContrast @{surface_risk_tau=.040; risk_gain=1.6})),
    (New-Spec 'bounded_tau050_gain160' 3 (Merge-Tune $lowContrast @{surface_risk_tau=.050; risk_gain=1.6})),
    (New-Spec 'bounded_tau075_gain160' 3 (Merge-Tune $lowContrast @{surface_risk_tau=.075; risk_gain=1.6})),
    (New-Spec 'bounded_tau020_gain120' 3 (Merge-Tune $lowContrast @{surface_risk_tau=.020; risk_gain=1.2})),
    (New-Spec 'bounded_tau020_gain200' 3 (Merge-Tune $lowContrast @{surface_risk_tau=.020; risk_gain=2.0}))
)

Write-Host "FlashBench bounded surface-risk matrix: $($specs.Count) focused configurations"
...
$report = [ordered]@{
    schema='FLASHGUARD_MATRIX/13'
    purpose='find the shortest bounded local-risk lifetime that recovers weak/moving flash suppression without reintroducing long trail recovery'
    ...
    invariant='production remains architecture 0; sub-50ms risk tau and >1.2 gain are benchmark-only overrides'
...
Write-Host "Bounded surface-risk matrix complete in $($screen.elapsed_ms) ms; primary passes: $($passing.Count)"
exit 0
