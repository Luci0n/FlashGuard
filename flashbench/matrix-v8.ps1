...
$screenSpecs = @(
    ...
    (New-Spec 'surface_risk_default' 3),
    (New-Spec 'surface_risk_low_contrast' 3 $lowContrast),
    (New-Spec 'surface_risk_low_contrast_gain075' 3 (Merge-Tune $lowContrast @{risk_gain=.75})),
    (New-Spec 'surface_risk_low_contrast_gain100' 3 (Merge-Tune $lowContrast @{risk_gain=1.0})),
    (New-Spec 'surface_risk_low_contrast_neutral12' 3 (Merge-Tune $lowContrast @{risk_neutral=.12})),
    (New-Spec 'surface_risk_low_contrast_neutral24' 3 (Merge-Tune $lowContrast @{risk_neutral=.24}))
)

Write-Host "FlashBench surface-risk architecture screen: $($screenSpecs.Count) configurations"
...
$report = [ordered]@{
    schema='FLASHGUARD_MATRIX/8'; purpose='surface-validated risk-only current-frame architecture experiment'
    architecture_modes=[ordered]@{
        '0'='legacy transported protected luminance'
        '1'='risk-only v1 with screen-space risk memory'
        '2'='risk-only v1 plus one-frame same-surface raw-source limiter'
        '3'='surface-validated risk-only current-frame mitigation'
    }
...
Write-Host "Selected surface-risk candidate: $($selected.name) (failed gates=$($selected.failed_architecture_gates))"
exit 0
