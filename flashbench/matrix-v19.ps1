...
function Invoke-Batch([object[]]$Specs) {
    ...
    $sw.Stop()
    if ($p.ExitCode -ne 0) { throw "Textureless-flow consensus matrix failed with exit code $($p.ExitCode)" }
...
$specs = @(
    ...
    (New-Spec 'textureless_flow_consensus_tau100_gain200' 11 (Merge-Tune $base @{surface_risk_tau=.100; risk_gain=2.0}))
)

Write-Host "FlashBench textureless-flow consensus matrix: $($specs.Count) accept/reject configurations"
$screen = Invoke-Batch $specs
$candidates = @($specs | ForEach-Object { Read-Candidate $_ $screen.directory })
$selected = @($candidates | Where-Object { $_.architecture_mode -eq 11 -and $_.primary_pass } | Select-Object -First 1)
$report = [ordered]@{
    schema='FLASHGUARD_MATRIX/19'
    purpose='direct accept/reject test of textureless moving-surface flow consensus as intrinsic moving-flash coverage'
    screening_resolution='320x180'
    screening_fps=30
    screen_elapsed_ms=$screen.elapsed_ms
    candidate_count=$candidates.Count
    invariant='benchmark architecture 11 only: production architecture 0 is unchanged; candidate transports only a validated motion vector to compare raw source and adds no display memory'
...
Write-Host "Textureless-flow consensus matrix complete in $($screen.elapsed_ms) ms"
exit 0
