...
function Invoke-Batch([object[]]$Specs) {
    ...
    $sw.Stop()
    if ($p.ExitCode -ne 0) { throw "Full stationary-repetition matrix failed with exit code $($p.ExitCode)" }
...
function Read-Candidate([object]$Spec, [string]$BatchDir) {
    ...
    $perceptual5 = [double](($per.cases | Where-Object { [double]$_.frequency_hz -eq 5.0 } |
        Measure-Object reduction -Minimum).Minimum)
    $weak4 = [double](($per.cases | Where-Object { [int]$_.delta_code -eq 4 } |
        Measure-Object reduction -Minimum).Minimum)
    ...
    [pscustomobject][ordered]@{
        ...
        perceptual_5hz_min_reduction=$perceptual5
        perceptual_delta4_min_reduction=$weak4
        pan_mae=$pan
...
Write-Host "FlashBench stationary-repetition matrix: $($specs.Count) configurations"
...
Write-Host "Stationary-repetition matrix complete in $($screen.elapsed_ms) ms"
exit 0
