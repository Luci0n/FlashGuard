...
function Invoke-Batch([object[]]$Specs) {
    ...
    $sw.Stop()
    if ($p.ExitCode -ne 0) { throw "Repetition-gated matrix failed with exit code $($p.ExitCode)" }
...
function Read-Candidate([object]$Spec, [string]$BatchDir) {
    ...
    [pscustomobject][ordered]@{
        ...
        repeated_memory_low=$Spec.repeated_memory_low
        repeated_memory_high=$Spec.repeated_memory_high
...
$mid = Merge-Tune $base @{repeated_memory_low=.30; repeated_memory_high=.55}
$strict = Merge-Tune $base @{repeated_memory_low=.40; repeated_memory_high=.65}
...
$specs = @(
    ...
    (New-Spec 'repeat_gate_early_gain160' 7 (Merge-Tune $early @{risk_gain=1.6})),
    (New-Spec 'repeat_gate_early_gain200' 7 (Merge-Tune $early @{risk_gain=2.0})),
    (New-Spec 'repeat_gate_default_gain160' 7 (Merge-Tune $defaultGate @{risk_gain=1.6})),
    (New-Spec 'repeat_gate_default_gain200' 7 (Merge-Tune $defaultGate @{risk_gain=2.0})),
    (New-Spec 'repeat_gate_default_gain240' 7 (Merge-Tune $defaultGate @{risk_gain=2.4})),
    (New-Spec 'repeat_gate_mid_gain200' 7 (Merge-Tune $mid @{risk_gain=2.0})),
    (New-Spec 'repeat_gate_strict_gain200' 7 (Merge-Tune $strict @{risk_gain=2.0})),
    (New-Spec 'repeat_gate_default_tau020_gain200' 7 (Merge-Tune $defaultGate @{surface_risk_tau=.020; risk_gain=2.0})),
    (New-Spec 'repeat_gate_early_tau020_gain240' 7 (Merge-Tune $early @{surface_risk_tau=.020; risk_gain=2.4}))
)

Write-Host "FlashBench repetition-gated surface-risk matrix: $($specs.Count) focused configurations"
...
$report = [ordered]@{
    schema='FLASHGUARD_MATRIX/14'
    purpose='test whether persistent local risk can be seeded only after repeated-flash evidence, preserving event-only trail clearing while recovering repeated/moving flash suppression'
    ...
    invariant='mode7 never seeds persistent local risk from an isolated current intrinsic event unless analyzer temporal risk has crossed the repeated-memory gate; production remains architecture 0'
...
Write-Host "Repetition-gated matrix complete in $($screen.elapsed_ms) ms; primary passes: $($passing.Count)"
exit 0
