...
function Invoke-Batch([object[]]$Specs) {
    ...
    $sw.Stop()
    if ($p.ExitCode -ne 0) { throw "Uniform-interior intrinsic matrix failed with exit code $($p.ExitCode)" }
...
$specs = @(
    ...
    (New-Spec 'uniform_interior_intrinsic_tau100_gain200' 12 (Merge-Tune $base @{surface_risk_tau=.100; risk_gain=2.0}))
)

Write-Host "FlashBench uniform-interior intrinsic matrix: $($specs.Count) accept/reject configurations"
$screen = Invoke-Batch $specs
$candidates = @($specs | ForEach-Object { Read-Candidate $_ $screen.directory })
$selected = @($candidates | Where-Object { $_.architecture_mode -eq 12 -and $_.primary_pass } | Select-Object -First 1)
$report = [ordered]@{
    schema='FLASHGUARD_MATRIX/20'
    purpose='direct accept/reject test of four-sided uniform-interior same-coordinate raw change as intrinsic moving-flash coverage'
    screening_resolution='320x180'
    screening_fps=30
    screen_elapsed_ms=$screen.elapsed_ms
    candidate_count=$candidates.Count
    invariant='benchmark architecture 12 only: production architecture 0 is unchanged; candidate uses current-frame spatial interior support plus previous raw source at the same screen coordinate and adds no display memory'
...
Write-Host "Uniform-interior intrinsic matrix complete in $($screen.elapsed_ms) ms"
exit 0
