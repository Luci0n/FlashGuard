...
function Invoke-Batch([object[]]$Specs) {
    ...
    $sw.Stop()
    if ($p.ExitCode -ne 0) { throw "Geometry-authority replay batch failed with exit code $($p.ExitCode)" }
...
function Read-Candidate([object]$Spec, [string]$BatchDir) {
    ...
    [pscustomobject][ordered]@{
        ...
        moving_infill_active=[double]$move.geometry_on_error_pixels.infill.active_fraction_on_error_pixels
        moving_vacated_active=[double]$move.geometry_on_error_pixels.vacated.active_fraction_on_error_pixels
        moving_effective_motion_active=[double]$move.geometry_on_error_pixels.effective_motion.active_fraction_on_error_pixels
        recovery_infill_active=[double]$recovery.geometry_on_error_pixels.infill.active_fraction_on_error_pixels
        recovery_vacated_active=[double]$recovery.geometry_on_error_pixels.vacated.active_fraction_on_error_pixels
        recovery_effective_motion_active=[double]$recovery.geometry_on_error_pixels.effective_motion.active_fraction_on_error_pixels
...
$specs = @(
    ...
    (New-Spec 'geometry_authoritative_neutral12' 5 $lowContrast)
)

Write-Host "FlashBench risk-independent disocclusion screen: $($specs.Count) focused configurations"
$screen = Invoke-Batch $specs
$candidates = @($specs | ForEach-Object { Read-Candidate $_ $screen.directory })
$report = [ordered]@{
    schema='FLASHGUARD_MATRIX/11'
    purpose='test whether disocclusion geometry must remain authoritative under high temporal risk'
...
Write-Host "Risk-independent disocclusion screen complete in $($screen.elapsed_ms) ms"
exit 0
