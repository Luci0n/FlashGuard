...
function Invoke-Batch([object[]]$Specs) {
    ...
    $sw.Stop()
    if ($p.ExitCode -ne 0) { throw "Persistent-risk ablation failed with exit code $($p.ExitCode)" }
...
$specs = @(
    ...
    (New-Spec 'event_only_neutral12' 6 $lowContrast),
    (New-Spec 'event_only_neutral12_gain100' 6 (Merge-Tune $lowContrast @{risk_gain=1.0})),
    (New-Spec 'event_only_neutral12_gain110' 6 (Merge-Tune $lowContrast @{risk_gain=1.1})),
    (New-Spec 'event_only_neutral12_gain120' 6 (Merge-Tune $lowContrast @{risk_gain=1.2}))
)

Write-Host "FlashBench persistent local-risk ablation: $($specs.Count) focused configurations"
$screen = Invoke-Batch $specs
$candidates = @($specs | ForEach-Object { Read-Candidate $_ $screen.directory })
$report = [ordered]@{
    schema='FLASHGUARD_MATRIX/12'
    purpose='ablate persistent local surface-risk while retaining current-frame intrinsic mitigation'
    screening_resolution='320x180'
    screen_elapsed_ms=$screen.elapsed_ms
    candidate_count=$candidates.Count
    invariant='mode6 never reads or writes persistent local surface-risk for display mitigation'
    screen_candidates=$candidates
}
$report | ConvertTo-Json -Depth 8 | Set-Content -Encoding utf8 (Join-Path $OutputDir 'matrix.json')
Write-Host "Persistent local-risk ablation complete in $($screen.elapsed_ms) ms"
exit 0
