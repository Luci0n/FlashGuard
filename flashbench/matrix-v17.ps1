...
function Invoke-Batch([object[]]$Specs) {
    ...
    $sw.Stop()
    if ($p.ExitCode -ne 0) { throw "Surface-propagation matrix failed with exit code $($p.ExitCode)" }
...
$specs = @(
    ...
    (New-Spec 'surface_propagation_tau100_gain200' 9 (Merge-Tune $base @{surface_risk_tau=.100; risk_gain=2.0}))
)

Write-Host "FlashBench surface-propagation matrix: $($specs.Count) accept/reject configurations"
$screen = Invoke-Batch $specs
$candidates = @($specs | ForEach-Object { Read-Candidate $_ $screen.directory })
$selected = @($candidates | Where-Object { $_.architecture_mode -eq 9 -and $_.primary_pass } | Select-Object -First 1)
$report = [ordered]@{
    schema='FLASHGUARD_MATRIX/17'
    purpose='direct accept/reject test of same-current-surface spatial propagation for moving intrinsic flash authority'
    screening_resolution='320x180'
    screening_fps=30
    screen_elapsed_ms=$screen.elapsed_ms
    candidate_count=$candidates.Count
    invariant='benchmark architecture 9 only: production architecture 0 is unchanged; no neighbor RGB/history is transported'
...
Write-Host "Surface-propagation matrix complete in $($screen.elapsed_ms) ms"
exit 0
